...
def generateGraphFeatures(inputFileName, outputFileName):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generates graph, node featues and labels of a certain circuit"
    )
    parser.add_argument(
        "-i ",
        "--InputFileName",
    ...
    parser.add_argument(
        "-o ",
        "--OutputFolder",
        help="The folder path to export the result to",
        type=str,
        required=True,
    )
    ...
    OutputFileName = args.OutputFolder
    generateGraphFeatures(InputFileName, OutputFileName)
